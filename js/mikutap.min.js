...
var MainManager = function () {
        function a() {
            w = aidn.window.width(),
                M = aidn.window.height(), x && (x.resize(w, M), k && k.resize())
        }
        ...
        function t() {
            g = 1, $("#scene_loading hr").css("display", "none"),
                $("#scene_loading hr").css("width", 0),
                $("#scene_loading").stop().fadeOut(200, "linear"),
                v ? ($("#scene_loading").stop().css("display", "none"),
                    $("#bt_back").stop().css("display", "none"),
                p && $("#bt_fs").stop().css("display", "none"),
        ...
        function bt_feedback(a) {
            (O = !O) ?
                ($("#bt_feedback a").text("按键效果: ON"),
                    aidn.util.setCookie("fb", "on", 2592e3)) :
                ($("#bt_feedback a").text("按键效果: OFF"),
        ...
        function bt_anime(a) {
            ...
            // var text,cookie;
            var flag;
            (animeIsOpen = !animeIsOpen) ? flag = "ON" : flag = "OFF";
            // ($("#bt_anime a").text("ANIME: ON"),
        ...
        function o() {
            if (P.update(), 1 == g && 0 > --L && r(), v && 1 == g) {
                ...
                if (l * s < a) {
                    ...
                    if (h += e - l, 0 <= (l = e) * s - a) {
                        for (e = Math.random(),
                                 a = 1,
                                 192 <= h ? h = 0 : 128 <= h ? (.7 > e && (a = 2),
                                 .5 > e && (a = 3)) : 64 <= h ? (.35 > e && (a = 2),
                                 .2 > e && (a = 3), .02 > e && (a = 0)) : 32 <= h ? (.35 > e && (a = 2),
                                 .24 > e && (a = 0)) : 0 <= h && .4 > e && (a = 0),
                                 e = 0; e < a; e++) {
                            d = c[f],
            ...
            // 自定义更新
            MIKU.customUpdate && MIKU.customUpdate();
            // 注册下一个触发, 即延时触发,
            // 如果使用 setTimeout 就不会后台挂起
            MIKU_CONFIG.BACK_PLAY = true;
            if (backPlay) {
                // 刷新率 为 60
                ...
                switch (g) {
                    case 1:
                        g = 0,
                            ...
                            $("#scene_main").stop().fadeOut(100, "linear"), r();
                        break;
                    default:
                        location.href = "https://github.com/SuCicada/mikutap"
                }
            }),
            $("#bt_feedback a").click(bt_feedback),
            $("#bt_backtrack a").click(i);
        $("#bt_anime a").click(bt_anime);
        $("#bt_backplay a").click(bt_backplay);
        $("#bt_autoplay a").click(bt_autoplay);
        ...
        k = new function () {
            ...
            function t(e) {
                ...
                if (r(n), e.originalEvent && e.originalEvent.touches) {
                    for (var t = e.originalEvent.touches.length, i = 1; i < t; i++) {
                        n = e.originalEvent.touches[i], n = a(n.pageX, n.pageY), r(n, 1)
                    }
                }
            }
            ...
            function r(a, e, n) {
                // console.log(a, e, n)
                v != a && (1 != e && (v = a),
                0 > v || (T.play(a % T.length, n),
                    L = 90,
                _ && (_ = !1, $("#bt_back").stop().fadeOut(200, "linear"),
                p && $("#bt_fs").stop().fadeOut(200, "linear"),
                    $("#scene_main .set").stop().fadeOut(200, "linear")),
                0 >= --F && (e = Math.floor(k.length * Math.random()),
                    n = e + P.length,
                    (e = G[n].length ? G[n].pop() : new k[e](X, n)).play(),
                    F = 12 * Math.random() + 6), e = a % P.length,
            ...
            this.start = function () {
                m || ($("#view").on("mousedown", t),
                    $(window).on("mousemove", i),
                    $(window).on("mouseup", o),
                    $(window).on("keydown", e),
                    $(window).on("keyup", n)),
                (m || window.TouchEvent) &&
                ($("#view").on("touchstart", t),
                    $(window).on("touchmove", i),
                    $(window).on("touchend", o)),
                ...
                P = [function (a, e) {
                    ...
                    var t = function (a) {
                        ...
                        this.play = function (a, t, i) {
                            n.visible = !0, n.clear();
                            var o = w * Math.random(), r = M * Math.random(),
                    ...
                    this.play = function () {
                        o.setChildIndex(r, o.children.length - 1), r.visible = !0, r.x = w / 2, r.y = M / 2;
                        var a = Math.floor(8 * Math.random() + 6), e = Math.min(w, M) * (.2 * Math.random() + .25),
                            i = 360 / a,
                            d = e * (.2 * Math.random() + .05), h = l(), c = Math.PI / 2 * Math.floor(4 * Math.random()),
                            f = 1;
                        .5 > Math.random() && (f = -1);
                        for (var u = 0; u < a; u++) {
                            var v, p = (f * i * u + c) * Math.PI / 180, m = e * Math.cos(p);
            ...
            _ = !1,
            L = 0,
            O = "off" == aidn.util.getCookie("fb");
        X = "off" == aidn.util.getCookie("bt");
        animeIsOpen = "off" == aidn.util.getCookie("animeIsClose");
        backPlay = true;
        autoPlay = true;
        i();
        // 初始先关掉
        ...
        m && $("#scene_main .attention").html("轻触 &amp; 滑动!"),
        u || $("#scene_top .attention").text("* 把声音开大点，然后享受吧！")) : ($("#ng").css("display", "block"),
            $(".ok").css("display", "none"),
        u || $("#ng .atten").html("抱歉，</br>你的浏览器不支此页面")),
            PIXI.utils._saidHello = !0, aidn.window.resize(a)
        this.k = k
